...
const generateMockCurrencies = (): Currency[] => [
  {
    ...
    stellarAssetIssuer: 'GA5ZSEJYB37JRC5AVCIAZDL2Y343STVCUCGAXWVG2NC7AL6H6HKEF7G2',
  ...
  },
  {
    ...
    stellarAssetIssuer: 'GC5R5Y5X5Z5W5V5U5T5S5R5Q5P5O5N5M5L5K5J5I5H5G5F5E5D5C5B5A',
    lastUpdated: new Date().toISOString(),
  },
];
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-white">Currency Management</h1>
            <p className="text-blue-200/60 mt-1">Configure supported fiat and crypto assets for rental payments.</p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6">
          <p className="text-blue-200/60 text-sm font-medium mb-1">Active Currencies</p>
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-bold text-white tabular-nums">12</span>
            <span className="text-emerald-400 text-xs font-semibold px-2 py-0.5 bg-emerald-400/10 rounded-full">+2 this month</span>
          </div>
        </div>
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6">
          <p className="text-blue-200/60 text-sm font-medium mb-1">Supported Anchors</p>
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-bold text-white tabular-nums">8</span>
            <span className="text-blue-400 text-xs font-semibold px-2 py-0.5 bg-blue-400/10 rounded-full">Stellar Network</span>
          </div>
        </div>
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6">
          <p className="text-blue-200/60 text-sm font-medium mb-1">Monthly Volume</p>
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-bold text-white tabular-nums">$245k</span>
            <span className="text-indigo-400 text-xs font-semibold px-2 py-0.5 bg-indigo-400/10 rounded-full">All assets</span>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-white/5 border-b border-white/10">
              <tr>
                <th className="px-8 py-5 text-sm font-semibold text-blue-200/70">Currency</th>
                <th className="px-8 py-5 text-sm font-semibold text-blue-200/70">Type</th>
                <th className="px-8 py-5 text-sm font-semibold text-blue-200/70">Anchor Info</th>
                <th className="px-8 py-5 text-sm font-semibold text-blue-200/70">Asset Details</th>
                <th className="px-8 py-5 text-sm font-semibold text-blue-200/70">Status</th>
                <th className="px-8 py-5 text-sm font-semibold text-blue-200/70 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {isLoading ? (
                ...
                <tr key={currency.id} className="hover:bg-white/10 transition-all duration-300 group">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="text-xs text-blue-200/40">Verified Partner</div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="font-mono text-xs text-blue-200/60 max-w-[150px] truncate" title={currency.stellarAssetIssuer}>
                      Issuer: {currency.stellarAssetIssuer.slice(0, 8)}...{currency.stellarAssetIssuer.slice(-8)}
                    </div>
                    <div className="text-[10px] text-blue-200/30 mt-1 uppercase tracking-wider">Precision: {currency.decimalPlaces}</div>
                  </td>
                  <td className="px-8 py-6">
                    <button 
                    ...
                    >
                      {currency.status === 'active' ? <CheckCircle2 size={14} /> : <XCircle size={14} />}
                      <span className="text-xs font-bold uppercase tracking-widest">{currency.status}</span>
                    </button>
                  </td>
        ...
        {!isLoading && filteredCurrencies.length === 0 && (
          <div className="py-20 text-center flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className="text-blue-200/60 text-sm">Try adjusting your search query</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-600/20 to-emerald-600/20 border border-white/10 rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-white mb-1">Exchange Rates Sync</h3>
            <p className="text-blue-200/70 text-sm">Real-time rates are automatically pulled from Stellar DEX and trusted anchors.</p>
          </div>
        </div>
