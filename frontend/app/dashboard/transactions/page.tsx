  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 p-6 md:p-10 space-y-10">
      ...
      <div className="relative group">
        ...
        <div className="relative bg-slate-900 border border-white/10 rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden">
          <div className="flex items-center gap-6 z-10">
            ...
            <div>
              <h1 className="text-4xl font-black tracking-tight text-white mb-2">Stellar Ledger</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatItem title="Success Rate" value={`${((stats.success / stats.total) * 100).toFixed(1)}%`} icon={CheckCircle2} color="emerald" />
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/5">
                <th className="px-8 py-6 text-sm font-black text-slate-500 uppercase tracking-[0.2em]">Transaction</th>
                <th className="px-8 py-6 text-sm font-black text-slate-500 uppercase tracking-[0.2em]">Asset/Amount</th>
                <th className="px-8 py-6 text-sm font-black text-slate-500 uppercase tracking-[0.2em]">From/To</th>
                <th className="px-8 py-6 text-sm font-black text-slate-500 uppercase tracking-[0.2em]">Status</th>
                <th className="px-8 py-6 text-sm font-black text-slate-500 uppercase tracking-[0.2em] text-right">Date</th>
              </tr>
            </thead>
              ...
              {isLoading ? (
                ...
                <tr key={tx.id} className="group hover:bg-white/5 transition-all duration-300">
                  <td className="px-8 py-8">
                    <div className="flex items-center gap-5">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border shadow-inner transition-transform group-hover:scale-110 duration-500 ${
                        ...
                        {tx.type === 'payment' ? <ArrowUpRight size={24} /> : <Layers size={24} />}
                      </div>
                      <div className="max-w-[180px]">
                        <div className="font-black text-white text-lg group-hover:text-blue-400 transition-colors uppercase tracking-wider">{tx.type.replace('_', ' ')}</div>
                        <div className="text-sm font-mono text-slate-500 truncate mt-1">{tx.hash.slice(0, 10)}...{tx.hash.slice(-10)}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-8">
                    <div className="flex flex-col gap-1">
                      {tx.amount ? (
                        <>
                          <div className="text-2xl font-black text-white tabular-nums">{tx.amount} <span className="text-blue-400 text-sm font-bold tracking-widest">{tx.assetCode}</span></div>
                          <div className="text-xs text-slate-500 font-bold uppercase tracking-widest">Network Fee: {tx.fee}</div>
                        </>
                      ) : (
                        <span className="text-slate-500 font-bold uppercase tracking-widest">N/A</span>
                      )}
                    </div>
                  </td>
                  <td className="px-8 py-8">
                    <div className="space-y-3">
                      <div className="flex items-center gap-2 group/addr cursor-pointer">
                        <div className="w-6 h-6 bg-slate-800 rounded-md flex items-center justify-center text-[10px] font-black group-hover/addr:bg-blue-500 transition-colors">FR</div>
                        <span className="text-sm font-mono text-slate-400 group-hover/addr:text-blue-400 transition-colors">{tx.from.slice(0, 8)}...{tx.from.slice(-8)}</span>
                      </div>
                      {tx.to && (
                         <div className="flex items-center gap-2 group/addr cursor-pointer">
                           <div className="w-6 h-6 bg-slate-800 rounded-md flex items-center justify-center text-[10px] font-black group-hover/addr:bg-emerald-500 transition-colors">TO</div>
                           <span className="text-sm font-mono text-slate-400 group-hover/addr:text-emerald-400 transition-colors">{tx.to.slice(0, 8)}...{tx.to.slice(-8)}</span>
                         </div>
                      )}
                    </div>
                  </td>
                  <td className="px-8 py-8">
                    <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-black uppercase tracking-[0.1em] border shadow-lg ${
                      tx.status === 'completed' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 shadow-emerald-900/10' :
                      tx.status === 'pending' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20 shadow-amber-900/10' :
                      'bg-red-500/10 text-red-400 border-red-500/20 shadow-red-900/10'
                    }`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${
                        tx.status === 'completed' ? 'bg-emerald-400 anim-pulse' :
                        tx.status === 'pending' ? 'bg-amber-400 animate-bounce' :
                  ...
                  <td className="px-8 py-8 text-right">
                    <div className="text-white font-bold">{new Date(tx.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
                    <div className="text-slate-500 text-sm mt-1">{new Date(tx.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="flex items-center justify-between p-2">
        <p className="text-slate-500 font-bold text-sm uppercase tracking-widest whitespace-nowrap">Showing 1 - 25 of 1,240 results</p>
        <div className="flex gap-4">
          ...
          <div className="flex items-center gap-2">
            {[1, 2, 3, '...', 12].map((n, i) => (
              <button key={i} className={`w-12 h-12 rounded-2xl font-black transition-all ${n === 1 ? 'bg-blue-600 text-white shadow-xl shadow-blue-900/20' : 'text-slate-500 hover:text-white'}`}>
                {n}
              </button>
            ))}
          </div>
    ...
    <div className="bg-slate-900/50 border border-white/5 rounded-[32px] p-8 hover:bg-slate-900 transition-all group overflow-hidden relative">
      ...
      <div className="flex items-start justify-between relative z-10">
        ...
        <div className="text-right">
          <p className="text-slate-500 font-black text-xs uppercase tracking-[0.2em] mb-2">{title}</p>
