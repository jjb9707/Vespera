...
const generateMockReferralStats = (): ReferralStats => {
  return {
    ...
    totalRewards: 80,
    referrals: [
      { id: '1', referredName: 'John Doe', status: 'REWARDED', createdAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(), rewardAmount: 10 },
      { id: '2', referredName: 'Jane Smith', status: 'REWARDED', createdAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(), rewardAmount: 10 },
      { id: '3', referredName: 'Michael Brown', status: 'COMPLETED', createdAt: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(), rewardAmount: 10 },
      { id: '4', referredName: 'Emily Davis', status: 'PENDING', createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString() },
      { id: '5', referredName: 'Chris Wilson', status: 'PENDING', createdAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString() },
    ],
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
          <div className="max-w-2xl space-y-6">
            ...
            <h1 className="text-5xl md:text-6xl font-black tracking-tight leading-tight">
              Invite Friends, <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-200 to-white">Earn USDC Rewards</span>
            </h1>
            <p className="text-xl text-blue-100/80 font-medium leading-relaxed">
              Share your love for Chioma with your fellow property owners and managers. 
              Earn <span className="text-white font-bold">10 USDC</span> for every successful referral that completes their first transaction.
            </p>
          </div>
          ...
          <div className="w-full lg:w-96 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[32px] p-8 shadow-2xl flex flex-col items-center text-center space-y-6 transform hover:scale-[1.02] transition-transform duration-500">
             ...
             <div>
               ...
               <div className="text-3xl font-black text-white tracking-tight leading-none uppercase">{stats.referralCode}</div>
             </div>
             <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-3 bg-slate-900/50 backdrop-blur-xl border border-white/5 rounded-[32px] p-8 space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex gap-2">
                <ShareButton icon={Twitter} color="bg-sky-500" onClick={shareOnTwitter} />
                <ShareButton icon={Facebook} color="bg-blue-600" onClick={() => toast.error('Facebook sharing coming soon!')} />
                <ShareButton icon={Linkedin} color="bg-indigo-600" onClick={() => toast.error('LinkedIn sharing coming soon!')} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl border border-white/5 rounded-[32px] p-8 space-y-8">
          ...
          <div className="space-y-8">
            <Step number="01" title="Share Code" description="Invite friends using your unique code or referral link." />
            <Step number="02" title="Friend Registers" description="They create an account and complete their profile." />
            <Step number="03" title="First Transaction" description="Once they complete their first asset-based transaction." />
            <div className="pt-4 border-t border-white/5">
              <div className="flex items-center gap-4 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl">
                ...
                <div>
                  <div className="text-emerald-400 font-black text-sm uppercase tracking-widest">Get Paid</div>
                  <div className="text-white font-bold">10 USDC credited to your wallet</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white/5">
                <th className="px-8 py-5 text-xs font-black text-slate-500 uppercase tracking-widest">User Involved</th>
                <th className="px-8 py-5 text-xs font-black text-slate-500 uppercase tracking-widest">Date Joined</th>
                <th className="px-8 py-5 text-xs font-black text-slate-500 uppercase tracking-widest">Current Status</th>
                <th className="px-8 py-5 text-xs font-black text-slate-500 uppercase tracking-widest text-right">Award</th>
              </tr>
            </thead>
                ...
                <tr key={referral.id} className="group hover:bg-white/5 transition-all duration-300">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="font-bold text-white group-hover:text-blue-400 transition-colors">{referral.referredName}</div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <td className="px-8 py-6">
                    <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${
                      ...
                      <div className={`w-1.5 h-1.5 rounded-full ${
                        referral.status === 'REWARDED' ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]' :
                        referral.status === 'PENDING' ? 'bg-amber-400 animate-pulse' :
                        referral.status === 'COMPLETED' ? 'bg-blue-400' : 'bg-slate-400'
                      }`}></div>
                      {referral.status}
                    </span>
                  </td>
                  <td className="px-8 py-6 text-right">
                    {referral.rewardAmount ? (
                      <div className="flex items-center justify-end gap-1.5">
                        <span className="text-xl font-black text-white">{referral.rewardAmount}</span>
                        <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">USDC</span>
                      </div>
                    ) : (
                      <span className="text-slate-600 font-bold uppercase tracking-widest text-xs">—</span>
                    )}
                  </td>
                </tr>
              ))}
  ...
  return (
    <div className="bg-slate-900/50 backdrop-blur-xl border border-white/5 rounded-[32px] p-8 hover:bg-slate-900 transition-all group relative overflow-hidden">
      ...
      <div className="relative z-10 space-y-4">
        <div className={`w-12 h-12 ${colors[color]} rounded-2xl flex items-center justify-center border shadow-lg group-hover:scale-110 transition-transform`}>
          <Icon size={24} />
        </div>
        <div>
          <p className="text-slate-500 font-black text-xs uppercase tracking-[0.2em] mb-1">{title}</p>
          <div className="flex items-baseline gap-1.5">
            <span className="text-4xl font-black text-white tabular-nums">{value}</span>
            {unit && <span className="text-sm font-black text-blue-400 uppercase tracking-widest">{unit}</span>}
          </div>
        </div>
      </div>
    </div>
...
function Step({ number, title, description }: { number: string, title: string, description: string }) {
  return (
    <div className="flex gap-6 group">
      <div className="text-3xl font-black text-blue-500/20 group-hover:text-blue-500 transition-colors duration-500 mt-1">{number}</div>
